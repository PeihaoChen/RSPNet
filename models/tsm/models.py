...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            from .vision import resnet_half
            self.base_model = getattr(resnet_half, base_model)(pretrained=True if self.pretrain == 'imagenet' else False, inplanes=64)
            if self.is_shift:
        ...
        #                 if self.print_spec:
        #                     logger.info('Adding temporal shift... {}'.format(m.use_res_connect))
    ...
    def get_feature(self, input, no_reshape=False):
        ...
        if not no_reshape:
            ...
            feat = self.base_model.get_feature(input.view((-1, sample_len) + input.size()[-2:]))
        else:
            feat = self.base_model.get_feature(input)
        feat = feat.view((bs, -1) + feat.size()[-3:])
        feat = feat.transpose(2, 1)
        return feat

    def get_output_and_feature(self, input, no_reshape=False):
        bs, c, t, h, w = input.shape
        if not no_reshape:
            sample_len = (3 if self.modality == "RGB" else 2) * self.new_length
            ...
            base_out, feat = self.base_model.get_output_and_feature(input.view((-1, sample_len) + input.size()[-2:]))
        else:
            base_out, feat = self.base_model.get_output_and_feature(input)
        feat = feat.view((bs, -1) + feat.size()[-3:])
        feat = feat.transpose(2, 1)
    ...
    def _construct_flow_model(self, base_model):
        ...
        if self.base_model_name == 'BNInception':
            ...
            logger.warning('#' * 30 + ' Warning! No Flow pretrained model is found')
        return base_model
